  ...
  return '<div class="amb-kiosk'+(active?' active':'')+'" style="--kc:'+tn.color+';--kcd:'+dark+'" onclick="ambOpenTenant(\''+tn.id+'\')">'+
    ...
    '<div class="amb-kiosk-bldg">'+
      '<div class="amb-kiosk-window">'+
        ...
        '<span class="amb-kiosk-verified"><i class="fas fa-circle-check"></i> '+esc(t('statVerified'))+'</span>'+
  ...
  $('ambProdModalBox').innerHTML =
    ...
    '<div class="amb-tp">'+
      ...
      '<div class="amb-tp-head">'+
        ...
        '<div><div class="amb-tp-name">'+esc(nameFor(tn))+' <i class="fas fa-circle-check" style="color:var(--success);font-size:.7rem"></i></div>'+
...
function buildShareText(title, sub){
...
window.ambShareProduct = function(pid){
  var p=P(pid); if(!p) return; var tn=tenant(p.tenantId);
  ambShareSheet(p.name, fmt(p.price)+' · '+nameFor(tn));
};
window.ambShareTenant = function(tid){
  var tn=tenant(tid); if(!tn) return;
  ambShareSheet(nameFor(tn), tn.cat+' · '+tn.floor+' · '+BUILDING.name);
};
function ambShareSheet(title, sub){
  var s=buildShareText(title, sub);
...
/* ── MY ORDERS (local history; backend will populate/sync later) ── */
window.ambOpenOrders = function(){
  var list = state.orders||[];
  var body;
  if(!list.length){
    body = '<div class="amb-orders-empty"><i class="fas fa-receipt"></i><div>'+L('noOrders')+'</div></div>';
  } else {
    body = '<div class="amb-orders-list">'+ list.map(function(o){
      ...
      return '<div class="amb-order-card">'+
        '<div class="amb-order-top"><span class="amb-order-ref">'+esc(o.ref)+'</span>'+
          '<span class="amb-order-status '+esc(o.status||'placed')+'">'+esc(o.status||'placed')+'</span></div>'+
        '<div class="amb-order-seller"><i class="fas fa-store"></i> '+esc(o.tenant.name)+' · '+esc(o.tenant.floor)+'</div>'+
        ...
        '<div class="amb-order-acts">'+
          '<button class="amb-order-reorder" onclick="ambReorder(\''+o.tenant.id+'\')"><i class="fas fa-rotate-right"></i> '+L('reorder')+'</button>'+
        '</div>'+
      '</div>';
    }).join('') +'</div>';
  }
  $('ambProdModalBox').innerHTML =
    ...
    '<div class="amb-orders-modal">'+
      '<div class="amb-orders-h"><i class="fas fa-receipt"></i> '+L('myOrders')+'</div>'+
      body+
    '</div>';
  $('ambProdModal').classList.add('on'); $('ambOverlay').classList.add('on'); document.body.style.overflow='hidden';
};
