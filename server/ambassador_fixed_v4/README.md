...
### v30.1 — three real bugs from v30 found and fixed, mobile kiosk redone

v30's rail/sheet shipped with genuine defects, found by testing with real
incremental scrolling and direct sheet-content checks rather than trusting a
single screenshot:

**The fixed rail was covering real content.** At every common desktop width
(1200–1440px) the rail sat directly on top of the first column of product
and tenant cards — confirmed by measuring bounding-box overlap. My earlier
"nudge the container 26px" fix was a leftover fragment; the correct rule
(copied faithfully from the Zefmesh reference this time) reserves 96px on
the container, hero, nav and footer between 1181–1527px wide. Verified with
zero overlap at 1200/1280/1366/1440px.

**Vacancies collapsed to 0 height inside the sheet.** An older rule that hid
the whole section on desktop when no jobs were posted — sensible when it
sat in the normal scroll — now made the Vacancies panel open to a blank
screen, since visiting it is an explicit click. Scoped the auto-hide to
exclude anything inside `.amb-sheet-page`; verified both the empty state
and a real posted job now render.

**Two sections were mislabeled, not missing.** "Management" pointed at a
Shop/Save/Play/Eat lifestyle graphic, not contact information — replaced
with a real Building Management page (phone, address, hours, feedback
route) built from the same `BUILDING` data the footer already uses.
"Reviews" pointed at the in-house feedback form; Google Maps reviews were
never fabricated, so it now also carries an honest link built from the
mall's real name and address (`google.com/maps/search`) rather than
invented review content. A "Floors" item — one of the eight originally
requested — was missing entirely and has been added, listing all 5 floors
and opening the existing floor panel.

**Mobile kiosk card rebuilt per explicit instruction.** The previous
shrink-everything approach is gone. The shop window and floor badge are
back to their original size; `.amb-kiosk-actions` gets `margin-top:auto`
inside the card's existing flex column so the buttons sit at a fixed
bottom position — measured identical (488px from the card top) on two
cards with different-length shop names, rather than following wherever
each card's text happened to end.

### v30 — icon rail, info sheet, full directory, compact mobile kiosks

**Mall information moved off the home page.** Amenities, Getting Here, Office
for Rent, Vacancies, Management, Media, Reviews and Partners no longer sit as
long sections on the storefront. They live in an overlay sheet reached from a
**fixed left icon rail** on desktop (with hover labels) and from the
**hamburger drawer** on phones — the pattern used by Zefmesh Grand Mall. The
home page now leads with shops and products.

Sections keep their original ids and stay in the document, so every existing
renderer still finds its element — the jobs board and services grid re-render
on demand when their page opens.

**"Full Directory" button** added under the hero's Building Directory, opening
a searchable list of all 130 shops grouped by floor, each tappable straight
through to that shop.

**Compact seller kiosks on phones.** The dark shop window was 150px tall,
so barely one card fit on screen. It is now 74px with a smaller awning and
logo, bringing the card from ~430px to 284px — two shops now visible at once.

*Two real bugs found in testing:* `floorsList` was called from a different
module than where it was defined (the full directory crashed on open), and
the mobile kiosk override was declared **before** the base rule so the
cascade discarded it — fixed by raising its specificity rather than relying
on source order.

*Note:* Font Awesome's CDN is blocked in my test sandbox, so icons appear
blank in my screenshots. The markup and icon names are valid for the FA 6.5.1
build the page loads, and they render normally on a live deployment.
